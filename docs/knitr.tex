...
\hypersetup{
  ...
  pdfauthor={著者: Xie, Yihui (谢益辉); 翻訳者: Katagiri, Satoshi},
  colorlinks=true,
  linkcolor=blue,
  filecolor=Maroon,
  citecolor=blue,
...
\date{ver. 1.3 (2021/03/21 19:34:35 JST, 本家最終更新時刻:
\href{https://github.com/rbind/yihui/tree/master/content/knitr}{2021/03/01
16:46:02 JST})}
...
\href{https://yihui.org/knitr/}{here}. \end{rmdimportant}
...
\textbf{訳注}: \textbf{R Markdown の場合,
フォーマット関数の処理内容によっては予期せぬ結果になることがあります}.
...
\begin{Shaded}
\begin{Highlighting}[]
\StringTok{"C:/Program Files/R/R{-}2.14.2/bin/Rscript.exe"} \ExtensionTok{{-}e} \StringTok{"knitr::knit2pdf(\textquotesingle{}\%.Rnw\textquotesingle{})"}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{Rscript}\NormalTok{ {-}e }\StringTok{"library(knitr); knit(\textquotesingle{}}\VariableTok{$GEDIT\_CURRENT\_DOCUMENT\_NAME}\StringTok{\textquotesingle{})"}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\#!/bin/bash}
\BuiltInTok{export} \VariableTok{PATH=$PATH}\NormalTok{:/usr/texbin:/usr/local/bin}
\ExtensionTok{Rscript}\NormalTok{ {-}e }\StringTok{"library(knitr); knit(\textquotesingle{}}\VariableTok{$1}\StringTok{\textquotesingle{})"}
\ExtensionTok{latexmk}\NormalTok{ {-}pdf }\StringTok{"}\VariableTok{$\{1\%}\NormalTok{.*}\VariableTok{\}}\StringTok{"}
\end{Highlighting}
\end{Shaded}
...
R Markdown のチャンクでも同様のことが可能です. \end{rmdtip}
...
\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\ExtensionTok{brew}\NormalTok{ install ffmpeg {-}{-}with{-}libvpx}
\end{Highlighting}
\end{Shaded}
