...
-   **`cache.globals`** (`NULL`; `character`): このチャンクで作成されない変数の名前のベクトルを指定します. このオプションは主に `autodep = TRUE` オプションをより正確に動作させたいときに使います. 例えば, チャンク `B` で使われているグローバル変数が チャンク `A` のローカル変数として使われているときなどです. グローバル変数の自動検出, つまり つまり, `cache.globals = NULL` または `TRUE` を指定してに失敗した際に使う場合, このオプションを使って手動でグローバル変数の名前を指定してください. 具体例として issue [\#1403](https://github.com/yihui/knitr/issues/1403) を参照してください. そして, `cache.globals = FALSE` は, ローカルかグローバルかに関係なく, コードチャンク内の全ての変数を検出することを意味します.
...
-   **`opts.label`**: (`NULL`; `character`). : このオプションは, チャンクオプションを他のオプションのテンプレート knitr::opts_template` や, コードチャンクから引き継ぐ機能があります. テンプレートの詳細は `?knitr::opts_template` を参照してください. このオプションは, ラベル名の代入された `character` 型のベクトルを受け取ります. このベクトル内の各要素に対して, **knitr** は最初に `knitr::opts_template` にあるラベルを探し出して, 見つかれば, 現在のチャンクにそのオプションテンプレートを適用しようとします. その後で, 文書内の他のチャンクラベルと名前の一致するものを探し出し, 見つかれば, 現在のチャンクに一致したチャンクのオプションを適用します. ここで参照されたコードチャンクは, 「被参照コードチャンク (refferenced code chunk)」と呼ばれます.
    ...
    チャンクオプションの優先度は, (1) ローカルチャンク (2) 被参照コードチャンク, (3) `knitr::opts_template` のテンプレート, (4) `knitr::opts_chunk` になります.
...
-   **`latex.tilde`** (`NULL`): `.Rnw` 文書の強調ブロックとして出力された場所の内側で, チルダを表示するためのコマンドです. 使用例は [the issue \#1992](https://github.com/yihui/knitr/issues/1992) を見てください.
