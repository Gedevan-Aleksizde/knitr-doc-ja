---
title: "『knitr: Rによる美麗で柔軟そして高速な動的レポート生成』日本語版"
...
<details>
<summary>運用手順</summary>

# 環境

再現性のため renv を使っている. `rmdja@development` を使っているので再現性は厳密ではない.
...
2. 更新差分を `https://github.com/yihui/yihui.org/compare/${lastcommit}..master` とかで見る.
    * TODO: どこで lastcommit を取得する?
3. 差分を見て変更された箇所に対応する
4. `bookdown::render_book(input = "source/", c("rmdja::gitbook_ja", "rmdja::pdf_book_ja"))` でビルドする.
    * 元のリポジトリはYihui氏のブログをビルドするためのものだが, ブログの全訳は作っていないので本体のワークフローを真似する必要はない.
